...
namespace AdobeConnectSDK.Common
{
    internal static class XmlSerializerHelpersGeneric
    {
        ...
        public static T FromXML<T>(XmlReader docReader, XmlRootAttribute xmlRootAttribute)
        {
            ...
            XmlAttributes attrs = new XmlAttributes {XmlRoot = xmlRootAttribute};
        ...
        public static T FromXML<T>(string xmlSource)
        {
            ...
            using (XmlTextReader reader = new XmlTextReader(new StringReader(xmlSource), true)) //AG: new XmlTextReader(xmlSource, XmlNodeType.Document, null))
            {
        ...
        public static string ToXML(object sourceDataObject, bool omitXmlDeclaration, bool omitXmlNamespaces)
        {
            if (sourceDataObject == null)
                throw new ArgumentNullException("sourceDataObject");
            if (!sourceDataObject.GetType().GetTypeInfo().IsSerializable)
            ...
            if (isIList && srcType.GetTypeInfo().IsGenericType)
            {
                if ((isIList && srcType.GetTypeInfo().IsGenericType) && srcType.GetGenericArguments().Length > 0)
                {
        ...
        private static XmlSerializer GetSerializerInstance(Type underlingType)
        {
            ...
            lock (locker)
            {
                ...
                {
                    cachedSerializer = new XmlSerializer(underlingType); //AG: new XmlSerializerFactory().CreateSerializer(underlingType);
        ...
        private static XmlSerializer GetSerializerInstance(Type underlingType, XmlAttributeOverrides xmlAttributeOverrides)
        {
            ...
            lock (locker)
            {
                ...
                if (!m_serializers.TryGetValue(cacheKey, out cachedSerializer))
                {
                    cachedSerializer = new XmlSerializer(underlingType, xmlAttributeOverrides); //AG: new XmlSerializerFactory().CreateSerializer(underlingType, xmlAttributeOverrides);
                    m_serializers.Add(cacheKey, cachedSerializer);
                }
            }
